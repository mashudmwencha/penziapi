...
class User(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            not self.pk and self.is_registered
        ):  # Change this condition to check if not registered
            self.is_registered = (
                False  # Set is_registered as False for new registrations
            )
...
class UserDescription(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="description"
    )
...
class Message(models.Model):
    ...
    def __str__(self):
        return (
            f"Message from {self.sender.name} to {self.receiver.name}: {self.message}"
        )
