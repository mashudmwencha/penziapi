...
from PenziApi.serializers import (
    UserDetailsSerializer,
    UserProfileSerializer,
    MergedUserDataSerializer,
)
...
def process_message(user, message):
    ...
    if message.lower() == "penzi" and not user.is_registered:
        ...
        message.lower().startswith("describe")
...
def handle_penzi_registered(user):
    return (
        "You are registered for dating. To search for a MPENZI, SMS match#age#town "
...
def handle_start_message(user, message):
    ...
    if message.lower().startswith("start#") and not user.is_registered:
        try:
            ...
            return {
                "status": "Invalid format. Please provide information in the format 'start#name#age#gender#county#town'"
            }, 400
        except ValidationError as e:
            return {"status": str(e)}, 400

        profile = UserProfile.objects.create(
            user=user, name=name, age=age, gender=gender, county=county, town=town
        )
        ...
        response_data["status"] = (
            f"Your profile has been created successfully, {username}. "
...
def handle_details_message(user, message):
    ...
    if message.lower().startswith("details#") and user.is_registered:
        try:
            _, level_of_education, profession, marital_status, religion, ethnicity = (
                message.split("#")
            )
        except ValueError:
            return {
                "status": "Invalid format. Please provide information in the format 'details#level_of_education#profession#marital_status#religion#ethnicity'"
            }, 400
        ...
        if last_details_time:
            ...
            ):  # Check if one minute has passed since 'details' message
                # Send a timeout response if one minute has elapsed since 'details' message
                response_data["status"] = (
                    f"You were registered for dating with your initial details. "
        ...
        response_data["status"] = (
            f"This is the last stage of registration. "
...
def handle_myself_message(user, message):
    ...
    if message.lower().startswith("myself ") and user.is_registered:
        ...
        response_data["status"] = (
            f"You are now registered for dating. "
...
def handle_match_message(user, message):
    ...
    if message.lower().startswith("match#") and user.is_registered:
        try:
            ...
            return {
                "status": "Invalid format. Please provide information in the format 'match#age_range#county'"
            }, 400
        ...
        if user_gender == "female":
            ...
            return {
                "status": "Your gender preference is not recognized. To register SMS start#name#age#gender#county#town to 22141."
            }, 400
        ...
        response_data["status"] = (
            f"We have {matching_users_count} {gender_display} who match your choice! "
        )

        if matching_users_count > 0:
            response_data[
                "status"
            ] += f"To get more details about a {gender_type}, SMS the match number e.g., 0722010203 to 22141"
...
def handle_next_message(user):
    ...
    if user.is_registered:
        ...
        ).exclude(id__in=displayed_matches)
        ...
        if len(remaining_matches) > 0:
            ...
            response_data["status"] = (
                "There are no more matches available at the moment. Try again later."
            )

    else:
        response_data["status"] = (
            "There are no matches available at the moment. Try again later."
        )
...
def handle_digit_message(user, message):
    ...
    if message.isdigit():
        ...
        match_profile = UserProfile.objects.filter(
            user__phone_number=match_phone_number
        ).first()
        ...
        if match_profile:
            ...
            response_data["status"] = (
                f"{match_profile.name} aged {match_profile.age}, {match_profile.county} County, {match_profile.town} town, "
            ...
            cache.set(
                f"match_user_{match_profile.user.id}_interested_in_{user.id}", True
            )
            cache.set(
                f"user_{user.id}_interested_in_match_{match_profile.user.id}", True
            )
...
def send_interest_message(
    match_user, user_profile_name, user_profile_age, user_profile_county, user
):
    ...
    if match_user_profile:
        ...
        message_body = (
            f"Hi {match_user_profile.name}, {user_profile_name} is interested in you and requested your details. "
        ...
        if user_profile:
            ...
            Message.objects.create(
                sender=user_profile, receiver=match_user_profile, message=message_body
            )
    ...
    try:
        if (
            ...
            if match_profile:
                ...
                response_data["status"] = (
                    f"{match_profile.name} describes themselves as  {match_description}"
                )
    ...
    try:
        if message.lower() == "yes" and user.is_registered:
            ...
            if interested_match_id:
                matched_profile = UserProfile.objects.filter(
                    user__id=interested_match_id
                ).first()
                match_interested_user = cache.get(
                    f"match_user_{matched_profile.user.id}_interested_in_{user.id}",
                    False,
                )

                if matched_profile and match_interested_user:
                    # Fetch details of the profile that user showed interest in
                    matched_details = UserDetails.objects.filter(
                    ...
                    response_data["status"] = (
                        f"{matched_profile.name} aged {matched_profile.age}, {matched_profile.county} County, {matched_profile.town} town, "
                        f"{level_of_education}, {profession}, {marital_status}, "
                        f"{religion}, {ethnicity}. Send DESCRIBE {matched_profile.user.phone_number} to get more details about {matched_profile.name}"
                    )
                else:
                    response_data["status"] = (
                        "Unable to find a matching profile or match interest not found."
                    )
            else:
...
def get_user_details(request):
    try:
        ...
        return Response({"message": "User details not found"}, status=400)


@api_view(["GET"])
def merge_user_data(request, user_id):
    try:
        user_profile = UserProfile.objects.get(user_id=user_id)
        user_details = UserDetails.objects.get(user_id=user_id)

        serializer = MergedUserDataSerializer(
            {
                "id": user_profile.id,
        ...
        return Response({"error": "UserDetails not found"}, status=400)
